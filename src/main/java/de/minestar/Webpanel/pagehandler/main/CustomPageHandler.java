...
public abstract class CustomPageHandler extends AbstractHTMLHandler {
	...
	protected CustomPageHandler(boolean needsLogin, Template template) {
		...
		if (needsLogin) {
			this.rpl_navigation = new TemplateReplacement("NAVIGATION",
					TemplateHandler.getTemplate("tpl_navi_on").getString());
		} else {
			this.rpl_navigation = new TemplateReplacement("NAVIGATION",
					TemplateHandler.getTemplate("tpl_navi_off").getString());
		}
	}

	@SuppressWarnings("unchecked")
	protected final void updateReplacements(HttpExchange http)
